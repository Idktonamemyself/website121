...
export function ProfileForm({ onClose }) {
    ...
    const [username, setUsername] = useState((user === null || user === void 0 ? void 0 : user.username) || '');
    const [bio, setBio] = useState((user === null || user === void 0 ? void 0 : user.bio) || '');
    const [avatarUrl, setAvatarUrl] = useState((user === null || user === void 0 ? void 0 : user.avatar_url) || '');
    ...
    const handleSubmit = (e) => __awaiter(this, void 0, void 0, function* () {
        ...
        setLoading(true);
        try {
            yield authService.updateProfile(user.id, { username, bio, avatar_url: avatarUrl });
      ...
      <div className="win98-window max-w-md w-full">
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-3">
          ...
          <div className="flex justify-center mb-4">
            {avatarUrl ? (<img src={avatarUrl} alt="Profile" className="w-20 h-20 shadow-win98-outset"/>) : (<div className="w-20 h-20 bg-[#808080] flex items-center justify-center shadow-win98-inset">
          ...
          <div>
            ...
            <input type="url" value={avatarUrl} onChange={(e) => setAvatarUrl(e.target.value)} className="win98-input w-full" placeholder="https://example.com/avatar.jpg"/>
          </div>

          <div>
            <label className="block text-sm font-bold mb-1">Bio:</label>
            <textarea value={bio} onChange={(e) => setBio(e.target.value)} className="win98-input w-full h-24 resize-none" placeholder="Tell us about yourself..."/>
          </div>
