        ...
        <div className="bg-[#c0c0c0] px-2 py-1 flex items-center gap-1 border-b border-[#808080]">
          ...
          <div className="flex items-center gap-2 ml-auto">
            <div className="flex items-center gap-1 bg-white shadow-win98-inset px-2 py-0.5">
              ...
              <input type="text" value={searchQuery} onChange={(e) => onSearchChange(e.target.value)} placeholder="Search posts..." className="bg-transparent border-0 outline-none text-xs w-32"/>
            </div>
            
            <a href="https://discord.gg/PqCwGaqJGk" target="_blank" rel="noopener noreferrer" className="win98-button flex items-center gap-1 py-0.5 px-2 bg-[#ff00ff] text-white no-underline">
              <MessageCircle className="w-3 h-3"/>
              <span className="text-xs">Discord</span>
            </a>
            
            {user ? (<div className="relative">
                <button onClick={() => setShowUserMenu(!showUserMenu)} className="win98-button flex items-center gap-1 py-0.5 px-2 bg-[#00ffff] text-black">
                  {user.avatar_url ? (<img src={user.avatar_url} alt="Avatar" className="w-4 h-4"/>) : (<User className="w-3 h-3"/>)}
                  <span className="text-xs font-bold">{user.username}</span>
                </button>
                
                {showUserMenu && (<div className="absolute right-0 mt-1 win98-window z-50">
                    <button onClick={() => { setShowProfile(true); setShowUserMenu(false); }} className="win98-button w-full flex items-center gap-2 py-1 px-3 text-left text-xs">
                      <User className="w-3 h-3"/>
                      Edit Profile
                    </button>
                    <button onClick={handleLogout} className="win98-button w-full flex items-center gap-2 py-1 px-3 text-left text-xs">
                      <LogOut className="w-3 h-3"/>
                      Logout
                    </button>
                  </div>)}
              </div>) : (<div className="flex gap-1">
                <button onClick={() => setShowLogin(true)} className="win98-button py-0.5 px-2 text-xs">
                  Login
                </button>
                <button onClick={() => setShowRegister(true)} className="win98-button py-0.5 px-2 text-xs bg-[#00ff00]">
                  Register
                </button>
              </div>)}
          </div>
